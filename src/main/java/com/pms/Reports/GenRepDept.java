...
@WebServlet(name = "GenRepDept", value = "/GenRepDept")
public class GenRepDept extends HttpServlet {

    private static Properties getConnectionData() {
        Properties props = new Properties();
        String fileName = "db.properties";

        try (InputStream in = GenRepDept.class.getClassLoader().getResourceAsStream(fileName)) {
            props.load(in);
        } catch (IOException ex) {
            Logger lgr = Logger.getLogger(GenRepDept.class.getName());
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (selectedDepts != null) {
            try {
                JsonArray reportData = generateDepartmentReport(selectedDepts);
    ...
    private JsonArray generateDepartmentReport(String[] departmentNames) {
        ...
        try (Connection con = DriverManager.getConnection(url, user, passwd)) {
            ...
            StringBuilder queryBuilder = new StringBuilder("SELECT * FROM Accepted WHERE Department IN (");
