...
@WebServlet(name = "GenRep", value = "/GenRep")
public class GenRep extends HttpServlet {
    ...
    private static Properties getConnectionData() {
        ...
        try (InputStream in = GenRep.class.getClassLoader().getResourceAsStream(fileName)) {
            props.load(in);
        } catch (IOException ex) {
            Logger lgr = Logger.getLogger(GenRep.class.getName());
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            JsonArray reportData = generateReport();
    ...
    private JsonArray generateReport() {
        ...
        try (Connection con = DriverManager.getConnection(url, user, passwd)) {
            String query = "SELECT * FROM Accepted";
